...
const SendResetPasswordOtp = async (req, res) => {
    ...
    try {
        ...
        if(userResult[0].registrationmethod === "google") {
            return res.status(401).json({ success: false, message: "Google authenticated users can't reset their password." });
        }
...
const setNewPassword = async (req, res) => {
    ...
    try {
        ...
        if (userResult.length === 0) {
            return res.status(410).json({success: false,message: "User not found."});
        }
        let otpData = otpStore.get(email);
        ...
        if (!otpData.validated) {
            return res.status(410).json({success: false,message: "OTP not verified. Please verify OTP before resetting password."});
        }
        ...
        if (addActiveSessionStatus.length === 0) {
         ...
         const newSecurityAlert = {
            ...
            browser_type: browser,
            type: "Login",
            message: "password reset and new device logged in",
            token: token,
        };
