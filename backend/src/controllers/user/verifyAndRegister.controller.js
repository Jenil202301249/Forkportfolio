...
export const register = async (req,res)=>{
    ...
    try{
        ...
        if(!record){
            return res.status(410).json({success: false,message: 'OTP Expired or invalid user'});
        }
        if(record.expiresAt<Date.now()||record.attempt<=0){
            otpStore.remove(email);
            return res.status(410).json({success: false,message: 'OTP has been expired'})
        }
        if(record.otp!==otp){
            record.attempt--;
            return res.status(400).json({success: false,message: `Invalid OTP, ${record.attempt} tries left`})
        }
        const user = await insertUser({name:record.name,email,Password:record.hashedPassword,method:"normal"});
        if(!user){
            return res.status(503).json({success: false,message: 'Database error occurred during registration'})
        }
        otpStore.remove(email);
